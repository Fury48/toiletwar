...
남자 화장실의 암묵적인 거리두기 규칙을 이용한 턴제 웹 게임 프로토타입입니다. 메인 화면에서 AI 대전과 PVP 대전을 선택할 수 있고, 현재 플레이 가능한 모드는 AI 대전입니다.

## 실행 방법

파일만 열어도 실행됩니다. 로컬 서버로 실행하려면 아래 명령을 사용하세요.
...
- 이후에는 연속된 U자형 나열에서 바로 붙어 있는 소변기에 사람이 없어야 둘 수 있습니다.
- 내가 둔 뒤 상대가 둘 수 있는 소변기가 없으면 체크메이트로 승리합니다.
- 현재 배치는 위쪽 11개, 오른쪽 세로 3개, 아래쪽 11개가 하나로 이어진 총 25개의 소변기를 사용합니다.
